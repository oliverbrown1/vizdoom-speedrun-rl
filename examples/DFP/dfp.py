...
if __name__ == "__main__":
    ...
    # Buffers to compute rolling statistics 
    reward_list = []
    loss_list = []
    life_buffer = []
    ...
    while not game.is_episode_finished():
        ...
        if (is_terminated):
            ...
            if GAME % 5 == 0:
                ...
                mean_loss = np.mean(loss_list[-5:])
                mean_score = np.mean(life_buffer[-5:])
                summary = tf.compat.v1.Summary()
                summary.value.add(tag='Mean Reward', simple_value=mean_reward)
                summary.value.add(tag='Mean Loss', simple_value=mean_loss)
                summary.value.add(tag='Mean Score', simple_value=mean_score)
                summary_writer.add_summary(summary, GAME)
